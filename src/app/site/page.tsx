    ...
    <>
      <section className="h-full w-full pt-20 relative flex items-center justify-center flex-col ">
        ...
        <div className="bg-gradient-to-r from-primary to-secondary-foreground text-transparent bg-clip-text relative  ">
          <h1 className="text-9xl font-bold text-center md:text-[300px]">WebZ</h1>
        </div>
        <div className="flex items-center justify-center relative md: mt-[-70px]">
          <Image src={'/assets/preview.png'} height={1200} width={1200} alt="plura" className="rounded-t-2xl rounded-tr-2xl border-2 border-muted" />
      ...
      <section className="flex justify-center flex-col gap-4 mt-20">
        ...
        <p className="text-muted-foreground text-center">
          WebZ is available in three different plans, choose the one that fits you best
        </p>
        <div className="flex items-center justify-center gap-4 flex-wrap mt-6">
          {pricingCards.map((card, index) => {
            return <Card key={card.title} className={clsx('w-[300px] flex flex-col justify-between', { "border-2 border-primary": card.title === "Unlimited Saas" })}>
              <CardHeader>
